...
def get_details(url):
    try:
        ...
        if content.strip():
            ...
            hasil_path = os.path.join(BASE_DIR, "hasil.txt")
...
def main_scraper(url, directory):
    ...
    for article in articles:
        ...
        if a_tag:
            ...
            hasil_path = os.path.join(BASE_DIR, "hasil.doc")
            if fungsi.does_file_exist(hasil_path) is False:
                fungsi.create_new_file(hasil_path)

            fungsi.write_to_file(hasil_path, "URL: " + link + "\n")
            get_details(link)
